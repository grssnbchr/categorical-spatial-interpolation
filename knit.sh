...
# find out checkpoint package date from index.Rmd
package_date_raw="$(grep -E "package_date\s<-\s\"[0-9]{4}\-[0-9]{2}\-[0-9]{2}\"" analysis/index.Rmd -o)"
# version_raw="$(grep -E "R_version\s<-\s\"[0-9]{1}\.[0-9]{1}\.[0-9]{1}\"" analysis/index.Rmd -o)"
...
# set correct checkpoint folder, so index.Rmd is knitted with historical rmarkdown package
